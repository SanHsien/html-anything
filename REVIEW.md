...
- 修復落地：同日 overlay 修正（pytest 下限、Issue 導流、gitignore、產品 `ci.yml` checkout 憑證、簡中 CONTRIBUTING 路徑）
- 主環境：Windows 11、PowerShell、Python 3.14（overlay gate）；產品 CI 仍是上游 Ubuntu `ci.yml`（Node 24 + pnpm）
- 狀態：可繼續當 Windows 維護線。**不是**產品安全審計通過證明。
...
### 本機 overlay gate（本輪修復後）

```text
pwsh -NoProfile -File tools\dev_check.ps1
→ 28 passed、WINDOWS DEV CHECK GREEN

python tools\check_dependency_freshness.py
→ pytest>=9.1 vs PyPI 9.1.1 OK；ruff>=0.16 vs 0.16.5 OK

git diff --check
→ 通過（僅 CRLF 提示，無 whitespace error）
```
...
- `next/src/lib/templates/skills/` 實有 **81** 個 skill 目錄；README 行銷數字仍寫 **75**。`detect.ts` 的 adapter 也多於 README 宣稱的 9 個 CLI。這是上游文案落差，不改英文 README。
...
| R-07 | Windows vitest 一筆 symlink EPERM | 記錄，不改產品測試。Ubuntu CI 仍會跑過 |
...
- 上游 `ci.yml` 繼續用未 pin 的 `actions/checkout@v4`／`setup-node@v4`。改 pin 會動產品契約字串；交給 Dependabot 開 PR 後再讀 diff。本輪只加 `persist-credentials: false`。
- 不把產品 README 翻成繁中主檔，也不改 README 的「75 skills / 9 CLIs」行銷數字（實測 81 個 skill 目錄、adapter 多於 9 個）。
- 不在 overlay gate 跑完整 Playwright。那是產品 CI 的工作。
- 現有上游 open PR／issue 本輪不逐筆讀 diff。watermark 不推進。
- 不修 `install-rejections.test.ts` 的 Windows `fs.symlink` EPERM。屬上游測試假設 Unix symlink 可用；要修需當次對話同意回貢。
- 不把 32 筆 CodeQL 告警當 overlay 工單清掉。多數反映「本機 spawn CLI + 寫 token + 預覽 HTML」的產品模型；真漏洞應走上游（含未讀的 #131）。
- 不改預覽 iframe 的 `allow-scripts allow-same-origin`。改了會動產品預覽契約。
- 英文 `CONTRIBUTING.md` 仍示範根目錄 `pnpm dev`／clone 上游；`guard.ts` 禁止根目錄 scripts。開頭 overlay 已指向 `pnpm -F`。簡中 export 列寫 `drafts-menu.tsx`、英文寫 `export-menu.tsx`，兩個檔都在 `next/src/components/`，本輪不為對齊再改語意。
- 不複製 Python 95% coverage／mutmut／Behave 進這個 Next.js repo。

## 尚未宣稱範圍

- 未在本機重跑 `pnpm -F @html-anything/e2e test`（Ubuntu CI 有跑過 `2104253`）。
- 未驗證任一 Agent CLI 的實際 spawn／SSE。
- 未把 CodeQL 32 筆告警當漏洞逐條驗證或關閉。
- 未審計 iframe sandbox、export sanitizer、skill 安裝 tar 路徑以外的安全面。
